	/**
	* @description Fetches information about the TOP.GG backendproject.
	* @author Krybskytten
	*/
...
module.exports = {
    name: "backendproject",
    description: 'General information about the top.gg backendproject from Crowdin.',
    usage: 'backendproject',
    ...
    execute(message, args) {
        if (message.channel.id != `${desired_channel}`) return;
                ...
                fetch(`https://api.crowdin.com/api/v2/projects/${crowdin_backendproject_id}`, {
                  method: 'GET', 
                ...
                // console.log(json);
                  ...
                  var hebrew = `${json.data.targetLanguageIds[9]}`
                  var hebrew2 = hebrew.replace("he", ":flag_il: ")
                  var hungarian = `${json.data.targetLanguageIds[10]}`
                  var hungarian2 = hungarian.replace("hu", ":flag_hu:")
                  var italian = `${json.data.targetLanguageIds[11]}`
                  var italian2 = italian.replace("it", ":flag_it:")
                  var japanese = `${json.data.targetLanguageIds[12]}`
                  var japanese2 = japanese.replace("ja", ":flag_jp:")
                  var korean = `${json.data.targetLanguageIds[13]}`
                  var korean2 = korean.replace("ko", ":flag_kr:")
                  var dutch = `${json.data.targetLanguageIds[14]}`
                  var dutch2 = dutch.replace("nl", ":flag_nl:")
                  var norwegian = `${json.data.targetLanguageIds[15]}`
                  var norwegian2 = norwegian.replace("no", ":flag_no:")
                  var polish = `${json.data.targetLanguageIds[16]}`
                  var polish2 = polish.replace("pl", ":flag_pl:")
                  var russian = `${json.data.targetLanguageIds[17]}`
                  var russian2 = russian.replace("ru", ":flag_ru:")
                  var swedish = `${json.data.targetLanguageIds[18]}`
                  ...
                  var indonesian = `${json.data.targetLanguageIds[23]}`
                  var indonesian2 = indonesian.replace("id", ":flag_id:")
                  var bengali = `${json.data.targetLanguageIds[24]}` 
                  var bengali2 = bengali.replace("bn", ":flag_bd:")
                  var azerbaijani = `${json.data.targetLanguageIds[25]}`
                  var azerbaijani2 = azerbaijani.replace("az", ":flag_az:")
                  var hindi = `${json.data.targetLanguageIds[26]}`
                  var hindi2 = hindi.replace("hi", ":flag_in:")  
                  var malay = `${json.data.targetLanguageIds[27]}`
